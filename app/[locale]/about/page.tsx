  ...
  const stats = [
    {
      icon: Users,
      value: '299+',
      label: locale === 'de' ? 'Zufriedene Schüler' : 'Memnun Öğrenci'
    },
    {
      icon: Star,
      value: '4.7',
      label: locale === 'de' ? 'Durchschnittliche Bewertung' : 'Ortalama Puan'
    },
    {
      icon: Trophy,
      value: '10+',
      label: locale === 'de' ? 'Jahre Erfahrung' : 'Yıl Deneyim'
    },
    {
      icon: Target,
      value: '95%',
      label: locale === 'de' ? 'Erfolgsquote' : 'Başarı Oranı'
    },
  ];
              ...
              {stats.map((stat, index) => {
                ...
                return (
                  <div key={index} className="text-center group">
                    <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-gold rounded-2xl mb-4 group-hover:scale-110 transition-transform">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <Link
                href={`/${locale}/contact`}
                className="inline-flex items-center px-8 py-4 bg-gradient-gold text-white font-bold rounded-xl hover:shadow-xl hover:scale-105 transition-all duration-300"
              >
                {locale === 'de' ? 'Jetzt kontaktieren' : 'Şimdi iletişime geçin'}
                <ArrowRight className="ml-2 w-5 h-5" />
              </Link>
            </div>

            <div className="relative">
              <div className="relative aspect-square bg-gradient-to-br from-primary-100 to-primary-50 rounded-3xl p-12 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <h3 className="text-3xl font-bold text-gray-900 mb-2">Fahrschule Karaaslan</h3>
                  <p className="text-primary-700 font-medium text-lg">
                    {locale === 'de' ? 'Seit vielen Jahren in Duisburg' : 'Duisburg\'da Uzun Yıllardır'}
                  </p>
                </div>
              </div>

              {/* Decorative Elements */}
              <div className="absolute -top-6 -right-6 w-24 h-24 bg-gradient-gold rounded-3xl opacity-20"></div>
              <div className="absolute -bottom-6 -left-6 w-32 h-32 bg-gradient-gold rounded-full opacity-10"></div>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {values.map((value, index) => {
              ...
              return (
                <div
                  key={index}
                  className="group bg-gradient-to-br from-gray-50 to-white p-8 rounded-3xl border-2 border-gray-100 hover:border-primary-300 transition-all hover:shadow-2xl hover:-translate-y-2 text-center"
                >
                  <div
                    className={`w-20 h-20 bg-gradient-to-br ${value.color} rounded-2xl flex items-center justify-center mx-auto mb-6 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}
                  >
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-white p-8 rounded-3xl shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-4">
                {locale === 'de' ? 'Erfahren & Professionell' : 'Deneyimli ve Profesyonel'}
              </h3>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-4">
                {locale === 'de' ? 'Individuelle Betreuung' : 'Bireysel Destek'}
              </h3>
              <p className="text-gray-600 leading-relaxed text-lg">
                {locale === 'de'
            ...
            <div className="bg-white p-8 rounded-3xl shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-4">
                {locale === 'de' ? 'Hohe Erfolgsquote' : 'Yüksek Başarı Oranı'}
              </h3>
              <p className="text-gray-600 leading-relaxed text-lg">
                {locale === 'de'
